...
try:
    ...
    app_fa.prepare(ctx_id=-1, det_thresh=0.20, det_size=(640, 640))
    AI_ENABLED = True
    print("InsightFace AI Engine (buffalo_sc / MobileFaceNet 512D) Initialized successfully.")
...
def calculate_confidence_score(sim: float) -> float:
    """
    Maps a raw ArcFace cosine similarity score (typically 0.35 to 0.70+ for valid matches)
    into a polished, intuitive confidence score ranging from 0.75 to 0.99 for live reporting.
    """
    if sim < 0.35:
        return round(float(sim), 2)
    # Map [0.35, 0.70] to [0.75, 0.99]
    clamped_sim = min(max(sim, 0.35), 0.70)
    mapped = 0.75 + ((clamped_sim - 0.35) / (0.70 - 0.35)) * (0.99 - 0.75)
